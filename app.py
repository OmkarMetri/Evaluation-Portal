...
def login():
    if request.method == "POST":
        ...
        c = conn.cursor()
        c.execute(
            "SELECT teacher_id FROM Teachers WHERE email = ? AND psw = ?",
            (user, password),
        )
        temp = c.fetchone()
        ...
        c.execute(
            "SELECT student_id FROM students WHERE email = ? AND psw = ?",
            (user, password),
        )
        temp = c.fetchone()
        conn.close()
...
def upload_assignment_teachers(tid):
    ...
    if request.method == "POST":
        ...
        deadline = datetime.strptime(dead, "%Y-%m-%dT%H:%M").strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        # print(deadline)
        ...
        note1 = multiprocessing.Process(
            target=notify.NewAssignment,
            name="new_" + str(guid),
            args=(
                dictionary["students"],
                dictionary["teacher"],
                dictionary["course"],
                [
                    url + "/PES/student/" + j[2] + "/assignments/" + str(guid)
                    for j in dictionary["students"]
                ],
                request.form.get("name"),
            ),
        )
        note2 = multiprocessing.Process(
            target=notify.Deadline,
            name=str(guid),
            args=(
                dictionary["students"],
                dictionary["teacher"],
...
def update_marks_teachers(tid, aid):
    ...
    c = conn.cursor()
    c.execute(
        "UPDATE Submissions SET assigned_marks = ? WHERE student_id = ? AND submission_id = ? AND assignment_id = ?",
        (
            dictionary["marks"],
            dictionary["student_id"],
            dictionary["submission_id"],
            dictionary["assignment_id"],
        ),
    )
    conn.commit()
    conn.close()
...
def finalize_marks_teachers(tid):
    ...
    for cid in list_courses:
        c.execute(
            "SELECT heading, max_marks, assignment_id FROM Assignments WHERE posted_to = ? AND posted_by=?",
            (cid, tid),
        )
        list_ass = c.fetchall()
    ...
    return render_template(
        "teachers/finalize_assignment_marks.html", passer=passer, id=tid, courses=cnames
    )
...
def view_submissions_teachers(tid, aid):
    ...
    c.execute(
        "SELECT max_marks, heading FROM Assignments WHERE assignment_id = ?", (aid,)
    )
    temp = c.fetchone()
    ...
    for student in students_now["students"]:
        marks = 0
        c.execute(
            'SELECT student_id, description, files, submission_id FROM Submissions WHERE type = "submission" AND assignment_id = ? AND student_id = ?',
            (aid, student[2]),
        )
        ...
        if len(submissions_now) != 0:
            c.execute(
                "SELECT assigned_marks FROM Submissions WHERE assignment_id = ? AND student_id = ?",
                (aid, student[2]),
            )
            marks = c.fetchone()
            if len(marks) > 0:
    ...
    conn.close()
    return render_template(
        "teachers/view_submissions.html",
        max_marks=max_marks,
        passer=passer,
        heading=heading,
        aid=aid,
...
def get_file_student(aid, sid, file):
    ...
    try:
        return send_file(
            "attachments/" + aid + "/" + sid + "/" + file, attachment_filename=file
        )
...
def list_assignments_students(sid):
    ...
    for tcid in list_tc:
        ...
        c.execute(
            "SELECT * FROM Assignments WHERE posted_by = ? AND posted_to = ?",
            (tid, cid),
        )
        list_ass = c.fetchall()
        ...
        for item in list_ass:
            ...
            # Needs unit testing
            c.execute(
                'SELECT student_id FROM Submissions WHERE assignment_id = ? AND type = "submission"',
                (item[0],),
            )
            list_sid = c.fetchall()
...
def delete_assignment_teachers(tid, aid):
    ...
    try:
        ...
        c = conn.cursor()
        c.execute(
            "DELETE FROM Assignments WHERE posted_by = ? AND assignment_id = ? ",
            (tid, aid),
        )
        conn.commit()
        conn.close()
...
def submit_page_assignment_students(sid, aid):
    ...
    if sid in list_sid:
        ...
        for item in list_ass:
            ...
            c.execute(
                'SELECT description, files, submitted_on FROM Submissions WHERE assignment_id = ? AND student_id = ? AND type ="submission"',
                (aid, sid),
            )
            sub = c.fetchone()
        ...
        c.execute(
            "SELECT assignment_id, submission_files FROM Assignments WHERE assignment_id = ?",
            (aid,),
        )
        list_ass = c.fetchall()
...
def delete_submission_students(sid, aid):
    ...
    try:
        ...
        c.execute(
            'SELECT submission_id FROM Submissions WHERE student_id = ? AND assignment_id = ? and type = "submission"',
            (sid, aid),
        )
        sub_id = c.fetchone()[0]
